...
def reset_cat():
    ...
    modovr = False
    rouovr = False
    r = urequestsget("http://10.0.0.56:1412/reset", timeout=2)
    data = r.content
    r.close()
    return data
...
def tick(timer):
    ...
    if volt2 < 12.2 and modpin and not modovr:
        r = urequestsget("http://10.0.0.56:1412/msolaroff", timeout=2)
        data = r.content
        r.close()
        sleep(0.5)
        get_pins()
...
async def index(request, response):
    ...
    if qs1 == _PASSWD: await response.send_file('main.py', max_age=0)
